...
// Every range above (and any future per-hotel `price` field added to
// hotelsByDestination) is a base rate for standard double occupancy — the
// same way a real hotel prices a room. Larger parties need an extra-bed/
// extra-person surcharge, same as any real property would charge. These
// are reasonable placeholder industry-standard figures, not numbers quoted
// by an actual property — replace them once real family/extra-bed rates
// come in, same as accommodationPriceRanges itself.
export const occupancyPricing = {
  baseOccupancy: 2,
  extraAdultUsd: 20,
  child: {
    // Below this age: no charge (shares existing bedding, common policy).
    freeUnderAge: 6,
    // From freeUnderAge up to (not including) this age: reduced rate.
    reducedUnderAge: 12,
    reducedUsd: 10,
    // This age and up: treated as an extra adult (needs their own bed).
    fullAgeUsd: 20,
  },
};
