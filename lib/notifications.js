...
export function notifyDriverBookingAdmin(contact, tripDetailsText, requestId, requestLink) {
  const message = `New driver booking request from ${contact.name} (${contact.email}, ${contact.phone}):\n\n${tripDetailsText}\n\nManage: ${requestLink}`;
  return send("contact@voyagees.com", "voyaGees Admin", message, requestId, "New driver booking request");
}
