...
function SetInitialView() {
  ...
  useEffect(() => {
    ...
    map.setView([7.8731, 80.7718], isMobile ? 7 : 8);
  }, [map]);
...
export default function ExploreSriLankaPage() {
  ...
  const center = useMemo(() => [7.8731, 80.7718], []);

  // ✅ THIS IS THE IMPORTANT FIX
  const filteredDestinations = useMemo(() => {
    if (filter === "all") return touristDestinations;
    return touristDestinations.filter((d) => d.type === filter);
  }, [filter]);
  ...
  return (
    <section style={{ padding: "40px 20px", background: "#f8fafc" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: "20px" }}>
        ...
        <p>Use filters to discover destinations based on your interest</p>
      </div>
        ...
        {filters.map((f) => (
          <button
            ...
            style={{
              ...
              background: filter === f.value ? "#0070f3" : "white",
      ...
      {hovered && (
        ...
        >
          ...
          <div style={{ padding: 12 }}>
            <h3>{hovered.name}</h3>
            <p style={{ fontSize: 13 }}>{hovered.description}</p>

            <button
              onClick={() => handleNavigate(hovered.slug)}
              style={{
                width: "100%",
                padding: 8,
                background: "#0070f3",
